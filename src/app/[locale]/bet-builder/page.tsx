...
export function generateMetadata({ params }: { params: { locale: string } }): Metadata {
  ...
  return {
    title: isFr ? "Generateur de Combis IA - PronoFoot AI" : "AI Combo Builder - PronoFoot AI",
    description: isFr
      ? "Generez des combis intelligents: sur, equilibre ou risque. Combine de paris generes par intelligence artificielle."
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 relative">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold text-white">
                {isFr ? "Generateur de Combis IA" : "AI Combo Builder"}
              </h1>
              <p className="text-indigo-300 text-sm mt-1">
                {isFr
                  ? "Combines intelligents generes par notre intelligence artificielle"
          ...
          <div className="flex flex-wrap gap-3 mt-6">
            <div className="flex items-center gap-2 bg-white/10 rounded-full px-4 py-1.5 text-sm text-white/80">
              ...
              {tipped.length} {isFr ? "matchs analyses" : "matches analyzed"}
            </div>
            <div className="flex items-center gap-2 bg-white/10 rounded-full px-4 py-1.5 text-sm text-white/80">
              <Layers className="w-4 h-4 text-indigo-400" />
              {combos.length} {isFr ? "combines generes" : "combos generated"}
            </div>
          </div>
        </div>
      </section>
        ...
        {combos.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-gray-200">
            ...
            <p className="text-gray-500 text-lg">
              {isFr
                ? "Pas assez de predictions aujourd'hui pour generer des combines. Revenez plus tard."
            ...
            {[
              {
                ...
                desc: isFr
                  ? "Notre IA analyse les probabilites, la forme et les statistiques de chaque match du jour."
                  : "Our AI analyzes probabilities, form, and statistics for every match today.",
              },
              {
                step: "02",
                title: isFr ? "Generation de Combines" : "Combo Generation",
                desc: isFr
                  ? "Trois combines sont generes: sur, equilibre et risque, selon votre profil de jeu."
